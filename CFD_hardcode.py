...
def rotatedPowerFunction(alpha, A, beta, mx, my, ma, up, numturbs,
    ...
    if heat:
        ...
        if interval_y > 0.01:
            y_start = min(ny) - (max(ny) - min(ny)) * 0.2 + interval_y / 2.
...
def Eval_Objective(mx, my, ma, A, B, numturb, heat=False):
    ...
    for i in range(bins * len(windsp)):  # for each wind direction and speed
        V, Q, VQ, mesh = create_mesh(mx, my, mz, ma, rad2, wind_cases[i][0])
...
def create_mesh(mx, my, mz, ma, rad2, alpha):
    # print('Ly ',Ly)
    # print('Lx ', Lx)
