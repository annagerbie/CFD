...
if shore == 'on':
    z0 = 0.05                 #Surface roughness, in m
...
def Eval_Objective(initial_num, z0, U0, Zref, alphah, ro, yrs, WCOE, condition, aif):
    ...
    #possible = 12.0 ** 3.
    #efficiency = windspeeds/possible
    #Ptot = efficiency * 0.3 * (12.0 ** 3.0)*initial_num
    #objective = cost / Ptot
   # PtotMW = Ptot / 1000000.0
    #cf = 0.4    #capacity factor
    #aep = Ptot * cf * 8760.0    #annual energy production (Watt hours / yr)
        #objective is over a period of years
    #print('The layout has a cost of: %s' % cost)
    #print('The layout produces: ', Ptot, 'Watts')    
    #profit (annual energy prod * energy cost in $/W * years) - O&M cost (0.007 $/kWh * annual energy prod * years / 1000 W/kW) - initial cost ($)
    #objective = (aep * 0.0001 * yrs) - cost   #removed "- (0.007 * aep * yrs / 1000.0)" since O&M is included above in Calculate_Cost
    objective = cost / Ptot
