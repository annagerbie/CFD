...
windsp = [7.5, 12.5]  # one wind speed
...
site_x = 1999.  # size of farm
site_y = 1999.  # size of farm
Lx = 6. * site_x  # size of mesh (NOT FARM)
Ly = 6 * site_y  # size of mesh (NOT FARM)
...
def createLayout(numturbs):
    ...
    if randStart:
        ...
        while not checkout:
            for n in range(numturbs):
                ...
                while checkx == 0 and reset < 50000:
                    CHECK2 = 0
                    x_opts = int(2. * site_x / mesh_width) + 1 + 1
                    # extra for rand.uniform
                    y_opts = int(2. * site_y / mesh_width) + 1 + 1
                    # extra for rand.uniform
                    mx[n] = int(rd.uniform(0, x_opts)) * mesh_width - site_x
                    my[n] = int(rd.uniform(0, y_opts)) * mesh_width - site_y
                    CHECK2 = Check_Interference(mx, my, n)
                    if CHECK2 != 1:
                        checkx = 1
                        # If there is no interference and the turbine can
                        # be placed, then choose new corresponding
                        # z-coordinate
                    else:
                        mx[n] = 0.
                        # If there is interference,
                        # move turbine back to origin
                        my[n] = 0.
                        reset += 1
                if reset == 50000:
                    print('resetting')
        ...
        mx = [-6680.980411, -6549.425159, -6380.075027, -6197.64205,
              -6015.935896, -5834.229741, -3668.29226, -3799.847528,
              -3435.70836, -3230.016971, -6651.907427, -6455.664786,
              -6265.236739, -6075.535515, -5810.971353, -5660.518655,
              -5505.705008, -5284.750318, -5082.693066, -4877.728514,
              -4743.992777, -4621.159409, -4735.270881, -4497.599216,
              -3722.077287, -3590.522019, -3462.600873, -3332.499253,
              -3274.353278, -3082.471558, -2533.718907, -2334.568937,
              -2151.409109, -1912.283778, -1732.758073, -1599.022325,
              -1551.778718]
        my = [-16892.41457, -16682.25616, -16486.55309, -16286.40222,
              -16109.60229, -15913.89922, -15251.17746, -15518.04528,
              -15084.38507, -14958.7348, -14737.4569, -14570.66451,
              -14410.54381, -14241.52752, -13962.42826, -13814.539,
              -13652.19441, -13447.59575, -13315.27378, -13175.16818,
              -12957.22612, -12707.03754, -11972.03907, -11891.97872,
              -11993.16611, -11794.12719, -11599.53607, -11399.3852,
              -11183.66704, -10937.92625, -10207.37558, -10036.1354,
              -9870.454957, -9691.431125, -9536.870177, -9323.375918,
              -9012.030123]
        mz = [HH] * len(mx)
        ...
        # Splits a listinto x and y coordinates,
    ...
    # print(my)
    plt.scatter(mx, my)
    plt.savefig('initial_layout.png')
    plt.close()
    return mx, my, mz
...
def createRotatedTurbineForce(mx, my, ma, A, beta, numturbs, alpha, V, mesh):
    ...
    if checkpts:
        ...
        n = [check_it(cos(alpha)*mx[i] - sin(alpha)*my[i],
                      sin(alpha)*mx[i] + cos(alpha)*my[i]) for i in range(numturbs)]
...
def Eval_Objective(mx, my, mz, ma, A, B, numturb, heat=False):
    ...
    for i in range(len(dirs) * len(windsp)):  # for each wind direction
        ...
        if heat:
            ...
            nx = [cos(wd)*mx[i] - sin(wd)*my[i] for i in range(numturbs)]
            ny = [sin(wd)*mx[i] + cos(wd)*my[i] for i in range(numturbs)]
            power_dev, heat_out = rotatedPowerFunction(wind_cases[i][0], A,
                                                       beta, mx, my, ma,
                                                       up_rot, numturbs, V,
                                                       mesh, True)
            print_heat(heat_out, nx, ny)
        else:
        ...
        cumulative_power = [(k * weights[i]
                             + jj) for k, jj in zip(power_dev,
                                                    cumulative_power)]
        print('time to evaluate bin: ', (time() - start) / 60., ' minutes')
...
def Rand_Vector(numturbs):
    ...
    for i in range(0, len(random_vec)):
        r = rd.randint(0, len(random_vec)-1)
    ...
    for h in range(0, 1):
        ...
        while step2 >= minstep:
            ...
            for j in range(0, len(random_vec)):
                ...
                # if my[i] >= 0.: #If commented out,
                # Don't adjust pattern order based on position in field
                if innerflag == 0 and flag == 0:
                    ...
                    if transflag == 1:
                        ...
                        if CHECK2 == 1:
                            ...
                            move2, turb_power2 = Eval_Objective(mx, my, mz, ma,
                                                                A, B, numturbs)
                ...
                if innerflag == 1 and flag == 0:
                    ...
                    if transflag == 1:
                        ...
                        if CHECK2 == 1:
                            ...
                            # if there is no interference, evaluate and store
                            move3, turb_power2 = Eval_Objective(mx, my, mz, ma,
                                                                A, B, numturbs)
                ...
                if innerflag == 2 and flag == 0:
                    ...
                    if transflag == 1:
                        ...
                        if CHECK2 == 1:
                            ...
                            # if there is no interference, evaluate and store
                            move4, turb_power2 = Eval_Objective(mx, my, mz, ma,
                                                                A, B, numturbs)
                ...
                if innerflag == 3 and flag == 0:
                    ...
                    if transflag == 1:
                        ...
                        if CHECK2 == 1:
                            ...
                            # if there is no interference, evaluate and store
                            move1, turb_power2 = Eval_Objective(mx, my, mz, ma,
                                                                A, B, numturbs)
                ...
                for b in range(0, num_pops):
                    ...
                    while flag == 0 and k < max_pop_tries:
                        ...
                        while checkx != 1:
                            ...
                            mx[min_turb] = int(rd.uniform(0, x_opts)) * mesh_width
                            my[min_turb] = int(rd.uniform(0, y_opts)) * mesh_width
                        ...
                        new_eval, turb_power2 = Eval_Objective(mx, my, mz, ma,
                                                               A, B, numturbs)
...
def check_layout(mx, my):
    ...
    with open('layouts.txt', 'a+') as layouts_file:
        ...
        for line in layouts_file:
            ...
            old_coords = [(old_coords[i],
                           old_coords[i + numturbs]) for i in range(numturbs)]
            for i in coords:
...
def print_heat(heat_out, nx, ny):
    plt.figure()
    plt.imshow(heat_out[0], cmap='hot', interpolation='nearest',
               extent=heat_out[1])
    plt.colorbar()
    ...
    plt.savefig('heat_out_' + str(now.month) + '_' + str(now.day)
                + '_' + str(now.hour) + '_' + str(now.minute) + '.png',
                bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    ma = [Constant(mm) for mm in 1. / 3. * np.ones(numturbs)]
    # axial induction factor
    # calculate the double integral of the smoothing kernel
    beta = integrate.dblquad(WTGdist, -3*radius, 3*radius,
                             lambda x: -3*radius, lambda x: 3*radius)
    ...
    # otherwise go strait to final plotting routine,
    # plot last inflow direction
    else:
    ...
    if heat_output:
        (Jfunc,
         cum_power) = Eval_Objective(mx_opt, my_opt, mz_opt, ma,
                                     A, B, numturbs, True)
    else:
        Jfunc, cum_power = Eval_Objective(mx_opt, my_opt, mz_opt, ma, A, B,
                                          numturbs)
    print('power output: ')
    print(Jfunc)
    data = [Jfunc]
